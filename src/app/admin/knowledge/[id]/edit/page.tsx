...
export default function EditKnowledgePage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/knowledge?limit=100`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/knowledge", {
        ...
        body: JSON.stringify({ id, ...form, images: form.images || null }),
      });
  ...
  if (loading) return <div className="text-center py-12 text-sm text-slate-400">加载中...</div>;

  return (
    <div className="max-w-2xl">
      <h1 className="text-lg font-semibold text-slate-800 mb-6">✏️ 编辑文章</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.images} onChange={(e) => update("images", e.target.value)}
              className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 focus:border-slate-500 outline-none" />
          </div>
        </div>
